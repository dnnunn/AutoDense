...
"""
UX-Optimized AutoDense interface with professional scientific UI patterns.
Implements enhanced accessibility, error handling, performance caching, and test IDs.
"""
...
def _try_run_ad_band_assist(pil_img: Image.Image, params: dict):
    """Run AutoDense pipeline; return dict with lanes, bands, overlay bytes, errors list."""
    ...
    try:
        ...
        if ADParams is not None:
            try:
                ...
                filt = {k: v for k, v in (kwargs or {}).items() if k in ann}
                ad_params = ADParams(**filt)  # type: ignore

        res = ad_pipeline_run(base, params=ad_params if ad_params is not None else kwargs)  # type: ignore
...
with tab1:
    st.markdown("""
    ## Lane Calibration & Setup
    Configure your gel parameters and establish lane boundaries using our two-point calibration system.
    """)
