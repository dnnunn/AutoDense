...
@with_error_boundary("Prerequisites Panel")
def render_prerequisites_panel(goto_tab: Optional[TabNavigationFunction] = None) -> bool:
    ...
    has_image: bool = safe_execute(
        "Image Status Check",
        lambda: st.session_state.res_uploaded_image is not None,
        default_return=False,
        show_error=False
    )
    has_calibration: bool = safe_execute(
        "Calibration Status Check",
        lambda: st.session_state.res_lane_boundaries is not None,
        default_return=False,
        show_error=False
    )
    ...
    with col1:
        @handle_ui_errors("Image Status Display", show_details=False)
    ...
    with col2:
        @handle_ui_errors("Calibration Status Display", show_details=False)
