...
from services.tools import dispatch_json_command
import os, json, time
        ...
        with progress_container:
            ...
            try:
                ...
                if not st.session_state.get('analysis_cancelled', False):
                    ...
                    # Expose a compact results dict for post-run chat/actions
                    ...
                    # Auto-suggest a practice after successful analysis
                    if not st.session_state.get("_auto_practice_prompt_shown", False) and os.getenv("AUTODENSE_AUTOPROMPT", "1") != "0":
                        st.session_state["_auto_practice_prompt_shown"] = True
                        min_support_env = int(os.getenv("AUTODENSE_PRACTICE_MIN_SUPPORT", "2"))
                        try:
                            sugg_json = dispatch_json_command(json.dumps({"tool":"suggest_practices","args":{"top_k":3,"min_support":min_support_env}}), st.session_state["analysis_results"])
                            data = json.loads(sugg_json).get("suggestions", [])
                        except Exception:
                            data = []
                        if data:
                            best = data[0]
                            with st.container(border=True):
                                st.markdown("#### 💡 Detected a routine you often run")
                                seq_str = " → ".join(best.get("sequence", []))
                                st.write(f"**Sequence (support={best.get('support',0)}):** {seq_str}")
                                default_name = f"practice_{int(time.time())}"
                                pname = st.text_input("Save as practice name", value=default_name, key="_autopractice_name")
                                desc = st.text_input("Description (optional)", value="Auto-suggested from recent usage", key="_autopractice_desc")
                                c1, c2 = st.columns([1,1])
                                with c1:
                                    new_min_support = st.number_input("Confidence threshold (min support)", min_value=1, max_value=20, value=min_support_env, step=1, key="_autopractice_support")
                                with c2:
                                    save_now = st.button("💾 Save Practice", use_container_width=True, key="_autopractice_save")
                                if save_now:
                                    rec = {"tool":"save_practice","args":{"name":pname,"desc":desc,"recipe":best.get("recipe", [])}}
                                    out = dispatch_json_command(json.dumps(rec), st.session_state["analysis_results"])
                                    st.success(f"Saved practice '{pname}'. You can run it anytime from the Chat tab.")
,
                    }
