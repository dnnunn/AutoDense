...
from .usage import read_recent
from .recommender import suggest_from_usage

def suggest_practices_tool(results: Dict[str, Any], **kwargs) -> str:
    top_k = int(kwargs.get("top_k", 5))
    min_support = int(kwargs.get("min_support", 2))
    events = read_recent(5000)
    suggestions = suggest_from_usage(events, top_k=top_k, min_support=min_support)
